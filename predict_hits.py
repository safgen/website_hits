...
def train_function(model, X, y, X_t, y_t, max_iter=100, verbose=False):
    if model == 'MLP':
        ...
        Model = SGDRegressor(max_iter=300, verbose=False, tol=1e-4)
    else:
...
if __name__ == "__main__":
    nRowsRead = None
    data = pd.read_csv('feature_engineered_data_with_hits.csv', nrows=nRowsRead).drop(columns=['Unnamed: 0'])
    data_unknown = pd.read_csv('feature_engineered_data_without_hits.csv', nrows=nRowsRead).drop(columns=['Unnamed: 0'])
    '''
    I am loading the feature encoded filed saved from the data-check.ipynb jupyter notebook. However, it can also be 
    regenerated by using the following class
    from Data_analysis import DataAnalysis  
    '''

    # test train split
    X_train, X_test, y_train, y_test = train_test_split(data.drop(columns=['row_num', 'hits']), data['hits'].values,
    ...
    output_data = data_unknown.drop(columns=['row_num', 'hits'])
